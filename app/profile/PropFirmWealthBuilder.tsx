...
export default function PropFirmWealthBuilder(){
 ...
 useEffect(()=>{(async()=>{const sb=createBrowserClient(process.env.NEXT_PUBLIC_SUPABASE_URL!,process.env.NEXT_PUBLIC_SUPABASE_ANON_KEY!);const {data:a}=await sb.auth.getUser();if(!a.user)return;const {data:u}=await sb.from("users").select("id").eq("auth_user_id",a.user.id).single();if(!u)return;setUserId(u.id);const {data:p}=await sb.from("prop_firm_wealth_plans").select("*").eq("user_id",u.id).eq("name","My Prop Firm Plan").maybeSingle();if(p)setPlan({...defaults,...p,monthly_returns:Array.isArray(p.monthly_returns)?p.monthly_returns:defaults.monthly_returns});})();},[]);
 ...
 <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(170px,1fr))",gap:12,marginTop:18}}>
  ...
  <label>Starting Capital<input type="number" min="1" value={plan.starting_capital} onChange={e=>set("starting_capital",e.target.value)}/></label>
 ...
 <p className="muted" style={{marginTop:16}}>Planning note: a negative or 0% month does not force a trade. The model compounds the assumptions you enter; income capacity is a planning estimate, not a guaranteed return or passive-income promise.</p>
