...
export default function ProfilePage() {
  ...
  useEffect(() => { const sb=createBrowserClient(process.env.NEXT_PUBLIC_SUPABASE_URL!,process.env.NEXT_PUBLIC_SUPABASE_ANON_KEY!); sb.auth.getUser().then(async ({data})=>{setEmail(data.user?.email??null); if(data.user){const {data:u}=await sb.from("users").select("mentorship_enrolled").eq("auth_user_id",data.user.id).single(); setEnrolled(Boolean(u?.mentorship_enrolled));} setLoading(false);}); }, []);
  ...
  return <main className="shell"><section className="card"><div className="section-label">PROFILE</div><h1 className="title">Your VaultTrades Profile</h1><p className="muted">Account and membership information.</p>
    ...
    <div className="card" style={{margin:"24px 0 0",border:"1px solid rgba(212,166,55,.35)"}}><div className="section-label">FOUNDERS MENTORSHIP</div><h2 style={{margin:"8px 0"}}>30 Days Founders Mentorship Program</h2><p className="muted">30 days · approximately 1 hour per day · recorded training.</p>{enrolled?<Link className="button" href="/founders-mentorship">Open Founders Mentorship</Link>:<><p className="muted">This area is visible to members, but access is locked until you enroll in the mentorship program.</p><button className="secondary" type="button" disabled>Locked — Enroll to Access</button></>}</div>
  </section></main>;
}
