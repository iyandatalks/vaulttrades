...
export async function POST() {
  try {
    ...
    const result = existing
      ? await context.admin.from("product_licenses").update(payload).eq("id", existing.id).select("id,status,start_at,end_at,access_key,platform,mt_login,broker_name,broker_server,purchased_product_code,entitlement_code,updated_at").single()
      : await context.admin.from("product_licenses").insert(payload).select("id,status,start_at,end_at,access_key,platform,mt_login,broker_name,broker_server,purchased_product_code,entitlement_code,updated_at").single();
