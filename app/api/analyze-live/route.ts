...
async function identifyAsset(apiKey: string, imageUrl: string): Promise<{ symbol: string; asset: string }> {
  const response = await fetch("https://api.openai.com/v1/responses", {
    method: "POST",
    headers: { "Content-Type": "application/json", Authorization: `Bearer ${apiKey}` },
    body: JSON.stringify({
      model: "gpt-4.1-mini",
      input: [{ role: "user", content: [{ type: "input_text", text: "Inspect this trading chart only to identify the visible instrument/symbol. Return the exact visible symbol if readable, otherwise a likely standard symbol such as XAU/USD, EUR/USD, BTC/USD, NAS100, US30, SPX. Never invent a symbol when the chart does not show one." }, { type: "input_image", image_url: imageUrl }] }],
...
export async function POST(request: Request) {
  try {
    ...
    // Identify the instrument from the chart, then source deterministic OHLCV independently of the screenshot.
    const identified = await identifyAsset(apiKey, imageUrl);
    ...
    }) : JSON.stringify({ provider: "Twelve Data", unavailable: true, reason: marketDataError });

    const prompt = `You are VaultTrades Analyzer. The screenshot is visual evidence; Twelve Data is the deterministic market-data evidence layer. Never pretend an indicator was read from the screenshot when the calculated Twelve Data value is available.
...
STRATEGY INDICATORS: ${selectedIndicators.join(", ") || "None"}
...
2. SMC: score BOS, CHoCH, Order Block, FVG, Liquidity Sweep, Displacement 1–10 with evidence. A NEW BUY/SELL requires at least TWO scores >=7.
...
STRATEGY LOGIC:
- The selected strategy is the primary thesis. Do not replace it with generic SMC or indicator logic.
- Indicators are supporting evidence, not the strategy. Do not impose a three-indicator quota. Select only indicators materially useful for this strategy.
...
- Use live Twelve Data values for current price, indicator readings and volatility where available. If Twelve Data is unavailable, say so explicitly and do not claim live confirmation.
...
Return JSON only.`;
    ...
    const responsePayload = {
      ...
      market: { asset: String(parsed.asset || identified.asset), timeframe: String(parsed.timeframe || selectedTimeframes.join(" + ")), marketCondition: String(parsed.marketCondition || ""), directionalBias: String(parsed.directionalBias || "") },
