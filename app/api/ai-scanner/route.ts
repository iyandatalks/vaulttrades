...
function inferMtfSnapshot(timeframe: "M15" | "M5", candles: Array<{ open: number; high: number; low: number; close: number }>): MtfSnapshot {
  ...
  const confirmation = direction === "BUY"
    ? recent.filter(x => x.close > x.open).length >= 2 && last.close >= recent[0].close
    : direction === "SELL"
      ? recent.filter(x => x.close < x.open).length >= 2 && last.close <= recent[0].close
      : false;
  ...
  return {
    ...
    currentPrice,
    direction,
    structureBreak: bullishBreak || bearishBreak,
    breakout: bullishBreak || bearishBreak,
    confirmation,
    state: timeframe === "M15" ? "CONFIRMATION" : "EXECUTION",
    support: supports.length ? Math.max(...supports) : null,
    resistance: resistances.length ? Math.min(...resistances) : null,
    structureReason: bullishBreak || bearishBreak
      ? `${timeframe} ${direction === "BUY" ? "bullish" : "bearish"} structure/breakout event with directional candle confirmation.`
      : `${timeframe} ${direction === "BUY" ? "bullish" : direction === "SELL" ? "bearish" : "mixed"} structure; fresh breakout confirmation is not yet present.`,
  };
}
...
function structuralProjection(direction: Direction, current: number, support: number | null, resistance: number | null, volatility: number | null) {
  ...
  if (direction === "BUY" && finite(support)) {
    const entry = support, stop = support - a, upper = finite(resistance) && resistance > entry ? resistance : entry + 2 * a;
    return { entry, stop, tp1: entry + (upper - entry) * 0.5, tp2: upper, tp3: upper + a, tp4: upper + 2 * a, reason: "BUY projection anchored to structural support." };
  }
  if (direction === "SELL" && finite(resistance)) {
    const entry = resistance, stop = resistance + a, lower = finite(support) && support < entry ? support : entry - 2 * a;
    return { entry, stop, tp1: entry - (entry - lower) * 0.5, tp2: lower, tp3: lower - a, tp4: lower - 2 * a, reason: "SELL projection anchored to structural resistance." };
  }
    ...
    if (mtfEnabled) {
      ...
      if (symbol) {
        const [m15Market, m5Market] = await Promise.all([
          getTwelveDataTimeSeries({ symbol, timeframe: "15m", outputsize: 100 }),
          getTwelveDataTimeSeries({ symbol, timeframe: "5m", outputsize: 100 }),
        ]);
        ...
        mtfEvidence = { enabled: true, htfTimeframe: selectedTimeframe, m15, m5, relationship: htfDirection === "BUY" && m15.direction === "BUY" && m5.direction === "BUY" && m15.confirmation && m5.confirmation ? "HTF BUY → M15 bullish confirmation → M5 bullish execution confirmation." : htfDirection === "SELL" && m15.direction === "SELL" && m5.direction === "SELL" && m15.confirmation && m5.confirmation ? "HTF SELL → M15 bearish confirmation → M5 bearish execution confirmation." : "MTF confirmation chain is incomplete." };
      }
    }
...
MTF LIFECYCLE FOR M30+
HTF strategy → setup detected → projected Entry/SL/TP1–TP4 → M15 confirmation → M5 execution confirmation → HTF final confirmation → actual confirmed entry → active HTF trade.
Regime is context only. Structure, breakout and confirmation lead direction. M15/M5 never rewrite HTF projected levels.

TP LIFECYCLE
The AI Scanner may display TP1, TP2, TP3 and TP4 as projected opportunity targets. The actual trade lifecycle is TP1-only: TP1 or SL completes the active cycle.

CONFIRMED ENTRY RULE
Do not call a setup confirmed merely because projected TP1 was reached. If projected TP1 is reached before actual confirmation, the setup expired/missed; do not retroactively enter it. Actual Entry exists only after the complete confirmation chain and is fixed thereafter.
...
Return JSON only. Preserve strategy-defined levels and distinguish DEVELOPING, CONFIRMED, ACTIVE, TP1_HIT, SL_HIT and CYCLE_COMPLETE.`;
    ...
    let sellProbability = finite(ai.sellProbability) ? Math.max(0, Math.min(100, ai.sellProbability)) : 50;
    const totalProbability = buyProbability + sellProbability || 1;
    buyProbability = buyProbability / totalProbability * 100;
    sellProbability = sellProbability / totalProbability * 100;
    ...
    const sourceProjectedEntry = finite(prior.projectedEntry) ? prior.projectedEntry : finite(prior.entry) ? prior.entry : null;
    const sourceProjectedStop = finite(prior.projectedStopLoss) ? prior.projectedStopLoss : finite(prior.stopLoss) ? prior.stopLoss : null;
    const sourceProjectedTp1 = finite(prior.projectedTp1) ? prior.projectedTp1 : finite(prior.tp1) ? prior.tp1 : null;
    const sourceProjectedTp2 = finite(prior.projectedTp2) ? prior.projectedTp2 : finite(prior.tp2) ? prior.tp2 : null;
    const sourceProjectedTp3 = finite(prior.projectedTp3) ? prior.projectedTp3 : null;
    const sourceProjectedTp4 = finite(prior.projectedTp4) ? prior.projectedTp4 : finite(prior.finalTp) ? prior.finalTp : null;
    ...
    const fallback = structuralProjection(direction, currentPrice, support, resistance, volatility);
    const projectedEntry = strategyId === "fibRetracement" ? (direction === "BUY" ? fibBuyLevel?.price ?? sourceProjectedEntry : direction === "SELL" ? fibSellLevel?.price ?? sourceProjectedEntry : sourceProjectedEntry) : sourceProjectedEntry ?? fallback?.entry ?? (finite(ai.entry) ? ai.entry : null);
    const projectedStopLoss = sourceProjectedStop ?? fallback?.stop ?? (finite(ai.stopLoss) ? ai.stopLoss : null);
    const projectedTp1 = sourceProjectedTp1 ?? fallback?.tp1 ?? (finite(ai.tp1) ? ai.tp1 : null);
    const projectedTp2 = sourceProjectedTp2 ?? fallback?.tp2 ?? (finite(ai.tp2) ? ai.tp2 : null);
    const projectedTp3 = sourceProjectedTp3 ?? fallback?.tp3 ?? (finite(ai.tp3) ? ai.tp3 : null);
    const projectedTp4 = sourceProjectedTp4 ?? fallback?.tp4 ?? (finite(ai.tp4) ? ai.tp4 : finite(ai.finalTp) ? ai.finalTp : null);
    ...
    const mtfChainConfirmed = mtfEnabled && direction !== "NO TRADE" && mtfEvidence.m15?.direction === direction && mtfEvidence.m5?.direction === direction && mtfEvidence.m15.confirmation && mtfEvidence.m5.confirmation;
    const htfFinalConfirmed = mtfEnabled ? htfStrategyConfirmed && mtfChainConfirmed : htfStrategyConfirmed;
    ...
    const cycleComplete = lifecycle.status === "TP1_HIT" || lifecycle.status === "SL_HIT" || lifecycle.status === "CYCLE_COMPLETE";
    const active = actualEntry !== null && lifecycle.status === "ACTIVE";
    const state = cycleComplete ? lifecycle.status : active ? "ACTIVE" : htfFinalConfirmed ? "CONFIRMED" : direction !== "NO TRADE" ? "CONFIRMATION_PENDING" : ai.analysisState;
    const projectionMath = math(direction, actualEntry ?? projectedEntry, projectedStopLoss, projectedTp1, currentPrice);
    const statusMessage = active ? `${direction} ACTIVE — confirmed entry ${actualEntry}. TP1 is the only lifecycle target.` : cycleComplete ? lifecycle.message : htfFinalConfirmed ? `${direction} CONFIRMED — actual entry ${actualEntry}.` : `${direction} DEVELOPING — confirmation conditions pending.`;

    return Response.json({
      ...ai,
      projectedDirection: direction, analysisState: state, statusMessage, buyProbability, sellProbability, probabilityDirection: direction,
      entry: projectedEntry, projectedEntry, actualEntry,
      stopLoss: projectedStopLoss, projectedStopLoss,
      tp1: projectedTp1, projectedTp1, tp2: projectedTp2, projectedTp2, tp3: projectedTp3, projectedTp3,
      tp4: projectedTp4, projectedTp4, finalTp: projectedTp4, projectedFinalTp: projectedTp4,
      confirmationPrice: actualEntry ?? ai.confirmationPrice,
      tp1Hit: lifecycle.tp1Hit, stopHit: lifecycle.stopHit, cycleStatus: lifecycle.status,
      ...
      volumeProfile: volume, rr: projectionMath.rr, priceValidation: projectionMath,
      isExecutable: htfFinalConfirmed && actualEntry !== null && projectionMath.valid && !cycleComplete,
      waitReason: statusMessage, tradeReason: statusMessage, invalidation: String(ai.invalidation || ""),
      mtf: mtfEvidence,
      mtfHierarchy: { enabled: mtfEvidence.enabled, htfTimeframe: mtfEvidence.htfTimeframe, htfSourceOfTruth: true, m15Role: "STRONGER_CONFIRMATION", m5Role: "EXECUTION_CONFIRMATION", lowerTimeframesRewriteHtfLevels: false, projectedEntrySource: "HTF_STRATEGY", actualEntryRule: "HTF_CONFIRMED_ONLY", independentLowerTimeframeCycles: true, lifecycleTarget: "TP1_ONLY" },
    });
  } catch (error) {
