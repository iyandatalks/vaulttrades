...
export async function POST(request: Request) {
  try {
    ...
    const selectedTimeframe = clean(prior?.market?.timeframe).toUpperCase(); const mtfEnabled = ["30M", "1H", "4H", "1D", "1W", "1M"].includes(selectedTimeframe);
    ...
    if (mtfEnabled) { const symbol = clean(prior?.market?.asset); if (symbol) { const [m15Market, m5Market] = await Promise.all([getTwelveDataTimeSeries({ symbol, timeframe: "15m", outputsize: 100 }), getTwelveDataTimeSeries({ symbol, timeframe: "5m", outputsize: 100 })]); const m15 = inferMtfSnapshot("M15", m15Market.candles); const m5 = inferMtfSnapshot("M5", m5Market.candles); const htfDirection: Direction = prior?.projectedDirection === "BUY" || prior?.direction === "BUY" ? "BUY" : prior?.projectedDirection === "SELL" || prior?.direction === "SELL" ? "SELL" : "NO TRADE"; mtfEvidence = { enabled: true, htfTimeframe: selectedTimeframe, m15, m5, relationship: htfDirection === "BUY" && m15.direction === "BUY" && m5.direction === "BUY" && m15.confirmation && m5.confirmation ? "HTF BUY → M15 bullish confirmation → M5 bullish execution confirmation." : htfDirection === "SELL" && m15.direction === "SELL" && m5.direction === "SELL" && m15.confirmation && m5.confirmation ? "HTF SELL → M15 bearish confirmation → M5 bearish execution confirmation." : "MTF confirmation chain is incomplete." }; } }
...
Do not call a setup confirmed merely because a projected target was reached. If projected TP2 is reached before actual confirmation, the setup expired/missed; do not retroactively enter it. Actual Entry exists only after the complete confirmation chain and is fixed thereafter.
...
Return JSON only. Preserve strategy-defined levels and distinguish DEVELOPING, CONFIRMED, ACTIVE, TP2_HIT, SL_HIT and CYCLE_COMPLETE.`;
    ...
    const priorActual = finite(prior.actualEntry) ? prior.actualEntry : null; const lifeActual = finite(lifecycleInput.actualEntry) ? lifecycleInput.actualEntry : null; const htfStrategyConfirmed = prior?.confirmed === true || prior?.status === "CONFIRMED" || prior?.status === "ACTIVE" || prior?.decision === "TRADE"; const mtfChainConfirmed = mtfEnabled && direction !== "NO TRADE" && mtfEvidence.m15?.direction === direction && mtfEvidence.m5?.direction === direction && mtfEvidence.m15.confirmation && mtfEvidence.m5.confirmation; const htfFinalConfirmed = mtfEnabled ? htfStrategyConfirmed && mtfChainConfirmed : htfStrategyConfirmed; const actualEntry = lifeActual ?? priorActual ?? (htfFinalConfirmed ? currentPrice : null);
