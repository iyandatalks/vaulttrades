...
2) ENTRY CONFIRMATION: This is the actual price event that permits entry after the setup exists. Use the selected strategy's entryConfirmationRule and designated confirmation timeframe. Report it separately as entryConfirmation.
3) UNIVERSAL VALIDATION: Analyzer Rules 1–6 remain unchanged and are evaluated separately as universalValidationPassed. A setup can have strategy conditions met and entry confirmation YES while still waiting on a universal validation gate.
...
Confirmation timeframe: ${profile.confirmationTimeframe}
...
A confirmed entry requires: strategyConditionsMet = true, entryConfirmation = true, universalValidationPassed = true, and valid trade geometry. Actual Entry exists only after these conditions are satisfied and is fixed thereafter.

SELECTED STRATEGY
${JSON.stringify({ id: strategyId, name: profile.name, focus: profile.focus, rules: profile.rules, indicatorSpecs: profile.indicatorSpecs, entryConfirmationRule: profile.entryConfirmationRule, confirmationTimeframe: profile.confirmationTimeframe, sourceRules })}
...
Return JSON only. Preserve strategy-defined levels and distinguish strategy setup, entry confirmation, universal validation and lifecycle. Never use ACTIVE/LIVE/TP1/TP2/SL/COMPLETED as entry confirmation.`;
    ...
    const projectedValidation = math(direction, projectedEntry, projectedStopLoss, projectedTp2 ?? projectedTp1, currentPrice);
    ...
    const confirmedSignal = direction !== "NO TRADE" && strategyConditionsMet && entryConfirmation && universalValidationPassed && projectedValidation.valid;
    const actualEntry = lifeActual ?? priorActual ?? (confirmedSignal ? currentPrice : null);
