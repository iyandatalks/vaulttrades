...
const ALLOWED_STRATEGIES = new Set(["adaptiveExecution", "ema20"]);
...
export async function GET() {
  ...
  return Response.json({
    config: data ?? { auth_user_id: user.id, enabled: false, observe_mode: true, forex_enabled: true, crypto_enabled: true, enabled_strategies: ["adaptiveExecution", "ema20"], trade_time_start: "01:30:00", trade_time_end: "08:45:00", timezone: "Africa/Johannesburg" },
...
export async function PATCH(request: Request) {
  ...
  if (body.enabled_strategies !== undefined) {
    if (!Array.isArray(body.enabled_strategies) || body.enabled_strategies.some((strategy) => typeof strategy !== "string" || !ALLOWED_STRATEGIES.has(strategy))) return Response.json({ error: "Only Adaptive Execution Engine and EMA20 Pullback Morning Engine can currently be automated." }, { status: 400 });
