...
const ALLOWED_STRATEGIES = new Set(["adaptiveAutomated", "emaAutomated"]);
...
export async function GET() {
  ...
  return Response.json({
    config: data ?? { auth_user_id: user.id, enabled: false, observe_mode: true, forex_enabled: true, crypto_enabled: true, enabled_strategies: ["adaptiveAutomated", "emaAutomated"], trade_time_start: "01:30:00", trade_time_end: "08:45:00", timezone: "Africa/Johannesburg" },
...
export async function PATCH(request: Request) {
  ...
  if (body.enabled_strategies !== undefined) {
    if (!Array.isArray(body.enabled_strategies) || body.enabled_strategies.some((strategy) => typeof strategy !== "string" || !ALLOWED_STRATEGIES.has(strategy))) return Response.json({ error: "Only Adaptive Automated and EMA Automated can currently be automated." }, { status: 400 });
  ...
  const service = createServiceClient();
  const { data, error } = await service.schema("scanner_automation").from("configs").upsert({ auth_user_id: user.id, ...updates, trade_time_start: "01:30:00", trade_time_end: "08:45:00", timezone: "Africa/Johannesburg" }, { onConflict: "auth_user_id" }).select("auth_user_id,enabled,observe_mode,forex_enabled,crypto_enabled,enabled_strategies,trade_time_start,trade_time_end,timezone,created_at,updated_at").single();
  if (error) return Response.json({ error: error.message }, { status: 500 });
