...
export async function GET(request: Request) {
  ...
  if (url.searchParams.get("supervision") === "1") {
    ...
    return Response.json({
      ...
      userId: user.id,
      tradingView: {
        received24h: events.data?.filter(e => e.stage === "RECEIVED").length ?? 0,
        persisted24h: events.data?.filter(e => e.stage === "PERSISTED").length ?? 0,
        rejected24h: events.data?.filter(e => e.status === "REJECTED").length ?? 0,
        failed24h: events.data?.filter(e => e.status === "FAILED").length ?? 0,
        latestEvent,
        latestSignal,
      ...
      executionQueue: {
        ...
        queued: queue.data?.filter(q => q.status === "queued").length ?? 0,
        claimed: queue.data?.filter(q => q.status === "claimed").length ?? 0,
        executed: queue.data?.filter(q => q.status === "executed").length ?? 0,
