...
export async function POST(request: Request) {
  ...
  const payload = { trade_id: clean(body?.trade_id) || null, market_category: clean(body?.market_category || body?.marketType || "FOREX").toUpperCase(), canonical_symbol: symbol, direction, strategy_id: selectedStrategy.id, strategy_name: selectedStrategy.name, timeframe, entry: finite(body?.entry) ? body.entry : null, stop_loss: finite(body?.stop_loss) ? body.stop_loss : null, tp1: finite(body?.tp1) ? body.tp1 : null, tp2: finite(body?.tp2) ? body.tp2 : null, tp3: finite(body?.tp3) ? body.tp3 : null, tp4: finite(body?.tp4) ? body.tp4 : null, tp5: finite(body?.tp5) ? body.tp5 : null, confirmation_timeframe: clean(body?.confirmation_timeframe || body?.confirmationTimeframe) || null, entry_quality: clean(body?.entry_quality || body?.entryQuality) || null, confidence: finite(body?.confidence) ? body.confidence : null, rr: finite(body?.rr) ? body.rr : null };
