...
export async function POST(req:Request){
 ...
 const {data:f}=await db.from("copy_followers").select("id,auth_user_id").eq("api_token_hash",sha(token)).maybeSingle();
 ...
 return NextResponse.json({ok:true,copyEnabled:true,accessUntil:access.endAt});
}