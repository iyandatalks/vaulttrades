...
export default function SignalsPage(){
  ...
  useEffect(()=>{ const load=async()=>{try{const r=await fetch("/api/signals",{cache:"no-store"});const x=await r.json();if(!r.ok)throw new Error(x.error||"Unable to load signals.");setSignals((x.signals||[]).filter((s:Signal)=>Date.now()-new Date(s.fired_at).getTime()<=SIGNAL_MAX_AGE_MS));setError("");}catch(e){setError(e instanceof Error?e.message:"Unable to load signals.");}};void load();const id=setInterval(()=>void load(),15000);return()=>clearInterval(id);},[]);
  useEffect(()=>{const id=setInterval(()=>setNow(Date.now()),30000);return()=>clearInterval(id);},[]);
  return <main className="shell">
    <section className="card"><div className="section-label">SIGNAL TAB · AUTOMATED LEDGER</div><h1 className="title">Automated Signals</h1><p className="muted">Only newly confirmed signals from the last 2 hours are shown. Signals older than 2 hours are removed from the execution-facing ledger.</p></section>
