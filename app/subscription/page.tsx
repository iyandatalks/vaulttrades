...
export default async function SubscriptionPage({ searchParams }: { searchParams: Promise<{ required?: string; result?: string }> }) {
  const params = await searchParams;
  const supabase = await createClient();
  const { data: { user } } = await supabase.auth.getUser();
  if (!user) redirect("/join?next=/subscription");

  const { data: profile } = await supabase.from("profiles").select("first_name,last_name,email").eq("id", user.id).maybeSingle();
  const { data: activeSubscription } = await supabase.from("subscriptions").select("provider,status").eq("user_id", user.id).eq("status", "active").limit(1).maybeSingle();
  ...
  return (
    <main style={{ minHeight: "calc(100vh - 61px)", background: "#050812", color: "#f4f6fb", display: "grid", placeItems: "center", padding: 24 }}>
      <section style={{ width: "100%", maxWidth: 660, border: "1px solid rgba(212,166,55,.25)", borderRadius: 14, padding: 32, background: "#0a0f1c" }}>
        ...
        {params.result === "cancelled" ? <p style={{ marginTop: 18, color: "#c9cfdd" }}>PayPal checkout was cancelled. Your account remains unchanged.</p> : null}
        <div style={{ marginTop: 24 }}><PayPalButton /></div>
        <div style={{ marginTop: 12, padding: "15px 18px", borderRadius: 9, textAlign: "center", background: "transparent", color: "#d4a637", border: "1px solid rgba(212,166,55,.35)", fontWeight: 800 }}>
          Pay with Yoco
          <div style={{ fontSize: 11, fontWeight: 500, marginTop: 4, color: "#7f8799" }}>Yoco connection will use this same VaultTrades account and subscription record.</div>
        </div>
