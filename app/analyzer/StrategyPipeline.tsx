...
type PipelineResult = {
  ...
  projection?: { zoneLow?: number | null; zoneHigh?: number | null; retestStatus?: string; confirmationStatus?: string; confirmationRequired?: string } | null;
  smcScores?: { BOS?: number; CHoCH?: number; OrderBlock?: number; FVG?: number; LiquiditySweep?: number; Displacement?: number } | null;
};

type Stage = { label: string; value: string; status?: "ok" | "pending" | "failed" | "info" };

const text = (value?: string | null, fallback = "Not clearly confirmed from the chart.") => value?.trim() || fallback;
const has = (result: PipelineResult, needle: string) => [...(result.confirmedConditions || []), ...(result.strategyAnalysis ? Object.values(result.strategyAnalysis) : [])].some(x => String(x).toLowerCase().includes(needle.toLowerCase()));
const score = (result: PipelineResult, key: keyof NonNullable<PipelineResult["smcScores"]>) => result.smcScores?.[key] ?? null;
const scoreText = (result: PipelineResult, key: keyof NonNullable<PipelineResult["smcScores"]>, label: string) => {
  const value = score(result, key);
  return value == null ? `${label}: not clearly scored` : `${label}: ${value}/10`;
};
const stageStatus = (value: string): Stage["status"] => /confirmed|valid|aligned|ok|complete|passed|bullish|bearish|yes|tap/i.test(value) ? "ok" : /pending|wait|none|not yet|await|missing|no clear|not confirmed/i.test(value) ? "pending" : "info";

function evidence(result: PipelineResult, label: string, preferred?: string[]) {
  const candidates = [
    ...(preferred || []),
    ...(result.confirmedConditions || []),
    ...(result.missingConditions || []),
  ].filter(Boolean);
  const hit = candidates.find(x => x.toLowerCase().includes(label.toLowerCase()));
  return hit || "No explicit confirmation recorded.";
}

function buildStages(result: PipelineResult): Stage[] {
  const id = result.strategy?.id || "";
  const a = result.strategyAnalysis;
  const zone = result.projection?.zoneLow != null && result.projection?.zoneHigh != null
    ? `${result.projection.zoneLow} – ${result.projection.zoneHigh}`
    : "No reliable setup zone identified";
  const bias = text(result.market?.directionalBias, "Neutral / unclear");

  if (id === "institutional") {
    return [
      { label: "Session structure", value: `${text(result.market?.session as string, "Session not visible")} · ${text(result.market?.timeframe, "timeframe unavailable")}` },
...
export function StrategyPipeline({ result }: { result: PipelineResult }) {
  ...
  const quality = result.currentState === "ACTIVE" ? "Active trade — confidence applies to the setup assessment, not a new entry." : undefined;
  return <section className="condition-box" style={{ marginTop: 16 }}>
    <div style={{ display: "flex", justifyContent: "space-between", gap: 12, alignItems: "center", flexWrap: "wrap", marginBottom: 12 }}>
      <div>
        ...
        <strong>{result.strategy?.name || "Strategy"} · {result.currentState || "WAITING"}</strong>
      </div>
      {quality && <span className="muted">{quality}</span>}
    </div>
    <div style={{ display: "grid", gap: 8 }}>
      {stages.map((stage, index) => <div key={`${stage.label}-${index}`} style={{ display: "grid", gridTemplateColumns: "minmax(145px, 0.32fr) 1fr", gap: 12, padding: "9px 0", borderTop: index ? "1px solid rgba(148,163,184,.12)" : undefined }}>
