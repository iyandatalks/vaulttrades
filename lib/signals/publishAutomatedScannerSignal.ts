...
export async function publishAutomatedScannerSignal(input: AutomatedScannerSignal) {
  ...
  const now = new Date().toISOString(); const tradeId = `VT-AUTO-${signalFingerprint.slice(0, 16).toUpperCase()}`;
