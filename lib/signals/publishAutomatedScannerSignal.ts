...
export async function publishAutomatedScannerSignal(input: AutomatedScannerSignal) {
  ...
  const timeframe = clean(input.timeframe).toUpperCase();
  ...
  if (!strategy) return { published: false, duplicate: false, error: `Unknown strategy '${input.strategyId}'.` };
  ...
  const confirmationBar = clean(input.analysis?.confirmationBar);
  // Identity is the confirmed setup, not the scheduler run. Re-running the
  // scanner against the same setup therefore cannot create another ledger row.
  const identity = {
    ...
    timeframe,
    confirmation_bar: confirmationBar || null,
    entry: scanner.actualEntry,
  };
  const signalFingerprint = hash(identity);

  const { data: existing, error: lookupError } = await supabase.from("scanner_signals")
    .select("*")
    .eq("auth_user_id", input.authUserId)
    .eq("signal_fingerprint", signalFingerprint)
    .maybeSingle();
  if (lookupError) return { published: false, duplicate: false, error: lookupError.message };
  if (existing) return { published: false, duplicate: true, signal: existing };

  const now = new Date().toISOString();
  const tradeId = `VT-AUTO-${signalFingerprint.slice(0, 16).toUpperCase()}`;

  // Lifecycle rule: only a newly confirmed opposite setup ends the prior
  // lifecycle. A same-direction continuation does not replace it.
  const { data: priorSignals, error: priorError } = await supabase.from("scanner_signals")
  ...
  for (const prior of priorSignals ?? []) {
    const lifecycle = prior.execution_payload && typeof prior.execution_payload === "object" ? prior.execution_payload as Record<string, unknown> : {};
    await supabase.from("scanner_signals").update({
      status: "INVALIDATED",
      completed_at: now,
      updated_at: now,
      execution_payload: { ...lifecycle, status: "INVALIDATED", lifecycle_status: "INVALIDATED", completed_at: now, invalidated_by: tradeId, invalidation_reason: `Confirmed opposite ${direction} setup on ${timeframe}.` },
    }).eq("id", prior.id).eq("auth_user_id", input.authUserId);
  }
  ...
  const executionPayload = {
    ...
    lifecycle_rule: "Ends on SL or confirmed opposite setup; projected TP levels do not invalidate a pre-entry setup.",
  };
  ...
  const row = {
    ...
    source_snapshot: { source: "AUTOMATED_SCANNER", automation_run_key: input.runKey, strategy_id: strategy.id, strategy_name: strategy.name, analysisState: scanner.analysisState, isExecutable: scanner.isExecutable, waitReason: scanner.waitReason, tradeReason: scanner.tradeReason, confirmationBar: confirmationBar || null, analysis: input.analysis ?? {} },
    fired_at: now,
    completed_at: null,
    updated_at: now,
  };
  ...
  if (error) {
    if (error.code === "23505") {
      const { data: duplicate } = await supabase.from("scanner_signals").select("*").eq("auth_user_id", input.authUserId).eq("signal_fingerprint", signalFingerprint).maybeSingle();
