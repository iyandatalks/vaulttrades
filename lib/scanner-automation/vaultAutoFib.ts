...
function buildM15Signal(c:Candle[],dxy:Candle[],now:number):VaultAutoFibResult|null{
  if(c.length<60)return null;
  const fib=vaultFibSignal(c,{confirmation:c,dxy,confirmationOffsetMs:15*60*1000-1});
  if(!fib)return null;
  const signalTime=c[fib.pullbackBar]?.time ?? 0;
  if(!signalTime || now-signalTime>MAX_SIGNAL_AGE_MS)return null;

  const entryConfirmation=confirmed(c,fib.side);
  ...
  const reason=[
    ...fib.reason,
    ...entryConfirmation.evidence,
    `Entry confirmation ${entryConfirmation.score}/100`,
    utAligned?`UT Bot ${fib.side} optional confluence`: 'UT Bot optional confluence not aligned',
  ];
...
export async function scanVaultAutoFib():Promise<VaultAutoFibResult[]>{
  const [m15,dxy15]=await Promise.all([
  ...
  const c=m15.candles.map(cv),dxy=dxy15.candles.map(cv),now=c.at(-1)?.time??Date.now();
  const signal=buildM15Signal(c,dxy,now);
