...
import { DEFAULT_ADAPTIVE_EXECUTION_CONFIG, evaluateAdaptiveExecution } from "../strategies/adaptiveExecution";
...
const SUPPORTED_STRATEGIES = new Set(["ema20", "adaptiveExecution"]);
...
export async function runScheduledScanner(job: Job) {
  ...
  const { data: rows, error: configError } = await supabase.schema("scanner_automation").from("configs")
    .select("auth_user_id,enabled,observe_mode,forex_enabled,crypto_enabled,enabled_strategies")
    .eq("enabled", true);
  ...
  const runKey = makeRunKey(job);
  const run = await startScannerRun(runKey, { timezone: TIMEZONE, job, executionMode: "OBSERVE_ONLY" });
  if (run.duplicate) return { status: "DUPLICATE", runKey };
  if (run.error) throw new Error(run.error);

  let marketsEvaluated = 0, strategiesEvaluated = 0, signalsDetected = 0, signalsPublished = 0, duplicateSignals = 0;
  const errors: string[] = [];

  try {
    for (const symbol of enabledSymbols(configs)) {
      const category = marketType(symbol);
      const strategyId = job === "EMA" ? "ema20" : "adaptiveExecution";
      ...
      try {
        if (job === "EMA") {
          ...
          const latest = runEma20Engine(snapshot.candles).at(-1);
          if (!latest || (!latest.newLong && !latest.newShort)) continue;
          const direction = latest.newLong ? "BUY" : "SELL";
          signalsDetected += 1;
          for (const config of eligibleUsers) {
            strategiesEvaluated += 1;
            const result = await publishAutomatedScannerSignal({
              authUserId: config.auth_user_id, runKey, marketType: category, symbol, timeframe: "5m", strategyId: "ema20",
              scanner: {
                ...
                tradeReason: "EMA20 authoritative engine produced a new signal transition on M5.", rr: 1.81,
              },
              analysis: { source: "EMA20 Pullback Morning Engine", observeMode: config.observe_mode, timeframe: "5m", marketType: category },
            });
          ...
          if (!isNew || !current.entry || !current.stopLoss || !current.tp1) continue;
          signalsDetected += 1;
          for (const config of eligibleUsers) {
            strategiesEvaluated += 1;
            const result = await publishAutomatedScannerSignal({
              authUserId: config.auth_user_id, runKey, marketType: category, symbol, timeframe, strategyId: "adaptiveExecution",
              scanner: {
                ...
                tradeReason: `${timeframe.toUpperCase()} independently satisfied the Adaptive Entry Confirmation rule.`,
                rr: current.risk && current.tp1 ? Math.abs(current.tp1 - current.entry) / current.risk : null,
              },
              analysis: { source: "Adaptive Execution Engine", authoritative: true, selectedTimeframe: timeframe.toUpperCase(), observeMode: config.observe_mode, result: current },
            });
            if (result.error) errors.push(`${config.auth_user_id}/${symbol}: ${result.error}`);
            if (result.published) signalsPublished += 1;
            if (result.duplicate) duplicateSignals += 1;
