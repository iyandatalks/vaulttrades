...
export type MarketDataCandle = {
  datetime: string; open: number; high: number; low: number; close: number; volume: number | null;
};

export type MarketInstrument = {
  symbol: string; name: string; type: string; exchange?: string; currency?: string;
};
...
const INTERVALS: Record<string, TwelveDataInterval> = {
  "1m": "1min", "5m": "5min", "15m": "15min", "30m": "30min", "1H": "1h", "4H": "4h", "1D": "1day", "1W": "1week", "1M": "1month",
};
...
export async function resolveTwelveDataSymbol(marketType: "FOREX" | "INDICES" | "CRYPTO" | "STOCKS", requested: string): Promise<string> {
  ...
  const exactAliases: Record<string, string> = {
    "XAU": "XAU/USD", "GOLD": "XAU/USD", "GOLD/USD": "XAU/USD", "XAUUSD": "XAU/USD",
    "XAG": "XAG/USD", "SILVER": "XAG/USD", "XAGUSD": "XAG/USD",
    "BTCUSD": "BTC/USD", "ETHUSD": "ETH/USD", "SOLUSD": "SOL/USD",
  };
  if (exactAliases[value]) return exactAliases[value];
  if (marketType === "FOREX" || marketType === "CRYPTO") return value;

  const aliases: Record<string, string> = {
    NASDAQ: "NASDAQ 100", NAS100: "NASDAQ 100", NDX: "NASDAQ 100",
    SPX: "S&P 500", SP500: "S&P 500", "S&P500": "S&P 500",
    DOW: "Dow Jones Industrial Average", DJIA: "Dow Jones Industrial Average",
  ...
  const resolved = typeof first?.symbol === "string" ? first.symbol : null;
  if (!resolved) throw new Error(`Twelve Data could not resolve the selected ${marketType.toLowerCase()} instrument.`);
  return resolved;
}
...
export async function getTwelveDataInstruments(marketType: "FOREX" | "INDICES" | "CRYPTO" | "STOCKS"): Promise<MarketInstrument[]> {
  ...
  const results = await Promise.all(queries.map(async query => {
    try { const payload = await twelveDataJson("symbol_search", { symbol: query, outputsize: "30" }); return Array.isArray(payload?.data) ? payload.data : []; } catch { return []; }
  }));
...
export async function getTwelveDataTimeSeries({ symbol, timeframe, outputsize = 120 }: { symbol: string; timeframe: string; outputsize?: number }): Promise<MarketDataSnapshot> {
  ...
  const candles: MarketDataCandle[] = values
    ...
    .sort((a: MarketDataCandle, b: MarketDataCandle) => {
      const aTime = Date.parse(a.datetime); const bTime = Date.parse(b.datetime);
