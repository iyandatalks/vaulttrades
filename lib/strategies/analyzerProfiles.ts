...
const UNIVERSAL_ANALYZER_RULES = [
  "RULE 1 — VISUAL CHART ANALYSIS: Always describe market structure as Uptrend, Downtrend, Ranging or Choppy; identify key support/resistance and recent price-action context before issuing a verdict.",
  "RULE 1 — CHOPPY MARKET RULE: More than 5 consecutive inside bars OR conflicting signals across 3 or more timeframes means NO TRADE due to choppy/insufficient clarity.",
  "RULE 2 — SMC DETECTION: Score BOS, CHoCH, Order Block, Fair Value Gap, Liquidity Sweep and Displacement from 1-10 when visible. Explain the evidence for every score. These observations supplement, but never replace, the selected strategy's authoritative source sequence.",
  "RULE 2 — SMC CONFLUENCE: Every NEW BUY or SELL setup requires at least 2 SMC signals scored 7 or higher. If fewer than two SMC signals are >=7, the final decision MUST be NO TRADE. Do not make this requirement conditional on strategy category.",
  "RULE 3 — SESSION & CONFLUENCE: Identify London, New York or Asian session when the chart/time information permits; note higher-timeframe alignment, visible news/fundamental context, and risk-on/risk-off sentiment without inventing unavailable news.",
  "RULE 3 — SESSION RULES: Asian-session trades require confluence >=9/10 and a major news event; London/NY overlap is preferred; pre-London 07:00-08:00 GMT is acceptable only for an exceptional setup.",
  "RULE 4 — PRICE LEVEL VALIDATION: For every BUY or SELL candidate, SL must be at least 0.1% from entry, entry must be within 0.5% of current price when current price is visible/derivable, and TP must be at least 2x the SL distance.",
  "RULE 4 — RISK MANAGEMENT: Maximum risk per trade is 1.5% of account equity. Do not claim this is satisfied unless position size/account-risk information supports the calculation; otherwise mark risk sizing as unverified and do not present it as validated.",
  "RULE 4 — RR VALIDATION: R:R must be mathematically validated at >=1:2. BUY risk = entry - SL and target distance = TP - entry. SELL risk = SL - entry and target distance = entry - TP. Never infer a valid RR from labels alone.",
  "RULE 4 — SIGNAL TYPES: BUY means all bullish source conditions plus all universal validations passed. SELL means all bearish source conditions plus all universal validations passed. NO TRADE means a required source condition or universal validation failed.",
  "RULE 5 — CONFIDENCE: 90-100 multiple strong confluences/perfect setup; 80-89 strong with 2-3 solid confirmations; 70-79 decent with minor issues; 60-69 marginal/wait; 50-59 weak/high failure risk; 30-49 poor/avoid; 10-29 very poor or image quality too low. Confidence must match evidence strength and may not be inflated merely because a source state exists.",
  "RULE 6 — QUALITY CHECK: Before finalizing, validate R:R math, SL/TP geometric direction, the 0.1% minimum SL distance, the 0.5% entry proximity when measurable, risk sizing when measurable, confidence versus confluence, and logical consistency between structure, price action, liquidity, momentum, volatility and the selected source lifecycle.",
  "RULE 6 — NO GENERIC FILLER: Do not output generic statements such as 'Ranging with no sustained higher highs or lower lows', 'Consolidation within defined horizontal range, no breakout', or 'Insufficient signals from EMA, ATR, RVOL for breakout' unless those facts are actually visible and relevant to the selected strategy. Explain the selected strategy's current state and the exact missing condition instead.",
  "RULE 6 — NO TRADE IS A DECISION, NOT AN EMPTY ANALYSIS: Even when the verdict is NO TRADE, describe the actual market state, source-specific setup state, confirmed conditions, missing conditions, invalidation, and the next actionable event/zone when visible."
];
