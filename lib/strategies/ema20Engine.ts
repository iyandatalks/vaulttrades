...
function emaSeries(values: Array<number | null>, length: number): Array<number | null> {
  ...
  for (let i = 0; i < values.length; i++) {
    if (previous === null) {
      ...
      if (seed !== null) { previous = seed; out[i] = seed; }
...
function pivotAt(candles: MarketDataCandle[], i: number, length: number, high: boolean): number | null {
  ...
  for (let j = 1; j <= length; j++) {
    ...
    } else if (value >= candles[pivotIndex - j].low || value >= candles[pivotIndex + j].low) return null;
  }
  return value;
}

/** Exact Pine expression: 200 * smiRelativeEMA / (smiRangeEMA / 2). */
function smiSeries(candles: MarketDataCandle[], length: number, smooth1: number, smooth2: number) {
  ...
  const range: Array<number | null> = Array(candles.length).fill(null);
...
const defaults: Required<Ema20EngineConfig> = {
  ema20Length: 20, slowLength: 105, atrLength: 14, atrTouch: 0.20, atrSLMultiplier: 2.23,
  riskReward: 1.81, pivotLength: 3, confirmationBars: 3, utKeyValue: 1.0, utATRLength: 10,
  smiLength: 7, smiK: 2, smiD: 2,
};
...
export function runEma20Engine(candles: MarketDataCandle[], input: Ema20EngineConfig = {}): Ema20EngineBar[] {
  ...
  let lastSwingHigh: number | null = null, previousSwingHigh: number | null = null;
  let lastSwingLow: number | null = null, previousSwingLow: number | null = null;
  let bullRejectHigh: number | null = null, bullRejectLow: number | null = null, bullRejectBar: number | null = null;
  let bearRejectHigh: number | null = null, bearRejectLow: number | null = null, bearRejectBar: number | null = null;
  ...
  for (let i = 0; i < candles.length; i++) {
    ...
    if (ph !== null) { previousSwingHigh = lastSwingHigh; lastSwingHigh = ph; }
    if (pl !== null) { previousSwingLow = lastSwingLow; lastSwingLow = pl; }
    ...
    if (bullReject) { bullRejectHigh = candle.high; bullRejectLow = candle.low; bullRejectBar = i; }
    if (bearReject) { bearRejectHigh = candle.high; bearRejectLow = candle.low; bearRejectBar = i; }
    if (bullRejectLow !== null && candle.close < bullRejectLow) { bullRejectHigh = null; bullRejectLow = null; bullRejectBar = null; }
    if (bearRejectHigh !== null && candle.close > bearRejectHigh) { bearRejectHigh = null; bearRejectLow = null; bearRejectBar = null; }
    if (bullRejectBar !== null && i - bullRejectBar > c.confirmationBars) { bullRejectHigh = null; bullRejectLow = null; bullRejectBar = null; }
    if (bearRejectBar !== null && i - bearRejectBar > c.confirmationBars) { bearRejectHigh = null; bearRejectLow = null; bearRejectBar = null; }
    ...
    if (utLoss !== null) {
      if (utStop === null) utStop = candle.close - utLoss;
    ...
    const smiMain = smi.main[i], smiSignal = smi.signal[i];
    ...
    let longEntry: number | null = null, longSL: number | null = null, longTP: number | null = null;
    let shortEntry: number | null = null, shortSL: number | null = null, shortTP: number | null = null;
    if (newLong) {
      ...
      bullRejectHigh = null; bullRejectLow = null; bullRejectBar = null;
    }
    if (newShort) {
      ...
      bearRejectHigh = null; bearRejectLow = null; bearRejectBar = null;
    }

    const d = new Date(candle.datetime);
    if (!Number.isNaN(d.getTime())) {
      const parts = new Intl.DateTimeFormat("en-US", { timeZone: "Africa/Johannesburg", hour: "2-digit", minute: "2-digit", hour12: false }).formatToParts(d);
...
export function evaluateEma20(candles: MarketDataCandle[], config: Ema20EngineConfig = {}): Ema20EngineBar | null {
  return runEma20Engine(candles, config).at(-1) ?? null;
}
