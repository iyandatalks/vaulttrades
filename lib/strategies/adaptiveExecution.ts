...
export type AdaptiveCandle = { open: number; high: number; low: number; close: number; volume?: number | null };
export type AdaptiveWeights = { trend: number; momentum: number; strength: number; structure: number; trigger: number };
export type AdaptiveExecutionConfig = {
  ...
  tp1RR: number; tp2RR: number; tp3RR: number; tp4RR: number; weights: AdaptiveWeights;
};
export type AdaptiveComponentScores = { trend: number; momentum: number; strength: number; structure: number; trigger: number };
export type AdaptiveExecutionResult = {
  direction: AdaptiveDirection; score: number; scores: AdaptiveComponentScores; confirmed: boolean;
...
export type AdaptiveMtfResult = {
  preferredTimeframe: PreferredTradeTimeframe | null; direction: AdaptiveDirection; executable: boolean;
  m15: AdaptiveExecutionResult; m5: AdaptiveExecutionResult; reason: string;
};
...
function atrSeries(candles: AdaptiveCandle[], length: number): Array<number | null> { return rmaSeries(trueRanges(candles), length); }
...
function total(values: AdaptiveComponentScores) { return values.trend + values.momentum + values.strength + values.structure + values.trigger; }
...
export function evaluateAdaptiveExecution(candles: AdaptiveCandle[], config: AdaptiveExecutionConfig = DEFAULT_ADAPTIVE_EXECUTION_CONFIG): AdaptiveExecutionResult {
  ...
  const tp1 = confirmed && risk !== null ? direction === "BUY" ? entry! + risk * config.tp1RR : entry! - risk * config.tp1RR : null; const tp2 = confirmed && risk !== null ? direction === "BUY" ? entry! + risk * config.tp2RR : entry! - risk * config.tp2RR : null; const tp3 = confirmed && risk !== null ? direction === "BUY" ? entry! + risk * config.tp3RR : entry! - risk * config.tp3RR : null; const tp4 = confirmed && risk !== null ? direction === "BUY" ? entry! + risk * config.tp4RR : entry! - risk * config.tp4RR : null;
...
export function evaluatePreferredM15M5(m15Candles: AdaptiveCandle[], m5Candles: AdaptiveCandle[], config: AdaptiveExecutionConfig = DEFAULT_ADAPTIVE_EXECUTION_CONFIG): AdaptiveMtfResult {
  const m15 = evaluateAdaptiveExecution(m15Candles, config); const m5 = evaluateAdaptiveExecution(m5Candles, config); const aligned = m15.confirmed && m5.confirmed && m15.direction === m5.direction;
  return { preferredTimeframe: aligned ? "M5" : m15.confirmed ? "M15" : null, direction: aligned ? m5.direction : "NO TRADE", executable: aligned, m15, m5, reason: aligned ? `M15 ${m15.direction} confirmation aligned with M5 ${m5.direction} execution.` : m15.confirmed ? `M15 ${m15.direction} confirmation is present; M5 execution confirmation is not aligned.` : "M15 confirmation is not established." };
}
